...
app = Flask(__name__)
app.secret_key = "second_brain_secret"

JSON_FILE = "comprehensive_decision_maker.json"
...
def save_result(username, category, score, total):
    ...
    cursor.execute("""
        INSERT INTO results (username, category, score, total_possible_score)
        VALUES (?, ?, ?, ?)
...
def login():
    if request.method == "POST":
        ...
        if login_or_register(username, password):
    ...
    <p class='subtitle'>✨ Make decisions with the vibe you deserve ✨</p>
...
def dashboard():
    ...
    for cat in data["categories"]:
        buttons += f"""
        <a href='/survey/{cat["category"]}'>
            <button class='category-btn'>{cat["category"].upper()}</button>
        </a>
        """

    content = f"""
    <h1>Wassup, {session['user']}! ✌️</h1>
    <p class='subtitle'>Select a situation below to conduct your survey:</p>
    {buttons}
...
def survey(category):
    ...
    data = load_data()
    selected_category = next((c for c in data["categories"] if c["category"] == category), None)

    if not selected_category:
        return redirect(url_for("dashboard"))

    questions = selected_category["questions"][:15]
    ...
    if request.method == "POST":
        ...
        save_result(session["user"], category, score, total)

        if "Career" in category:
            advice = (
                "Freelance / Entrepreneurship recommended."
                if score > total * 0.6
                else "Corporate Professional path recommended."
            )
        elif "Financial" in category:
            advice = (
                "Aggressive growth investments recommended."
                if score > total * 0.6
                else "Low-risk stable investments recommended."
            )
        else:
            advice = "Focus on balanced productivity and wellness."

        return redirect(url_for("advice", text=advice))

    form_html = f"<h1>{category}</h1><form method='POST'>"
...
@app.route("/advice")
def advice():
    advice_text = request.args.get("text", "")
    content = f"""
    <h1>🤖 AI DECISION & ADVICE SUMMARY</h1>
    <div class='advice-box'>{advice_text}</div>
